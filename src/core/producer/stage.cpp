...
struct stage::impl : public std::enable_shared_from_this<impl>
{
  ...
  public:
    ...
    {
        return flatten(executor_.begin_invoke([=, this] { return get_layer(index).foreground()->call(params).share(); }));
    }
    std::future<std::wstring> callbg(int index, const std::vector<std::wstring>& params)
    {
        return flatten(executor_.begin_invoke([=, this] { return get_layer(index).background()->call(params).share(); }));
    }
...
{
    return executor_.begin_invoke([=, this]() { return stage_->swap_layer(index, other_index, swap_transforms).get(); });
}
std::future<void>
...
{
    return executor_.begin_invoke([=, this]() -> std::shared_ptr<frame_producer> { return stage_->foreground(index).get(); });
}
std::future<std::shared_ptr<frame_producer>> stage_delayed::background(int index)
{
    return executor_.begin_invoke([=, this]() -> std::shared_ptr<frame_producer> { return stage_->background(index).get(); });
}
