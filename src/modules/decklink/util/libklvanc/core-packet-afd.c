...
int klvanc_dump_AFD(struct klvanc_context_s *ctx, void *p)
{
	...
	struct klvanc_packet_afd_s *pkt = (klvanc_packet_afd_s*)p;
			      ...
			      struct klvanc_packet_header_s *hdr, void **pp)
{
	...
	struct klvanc_packet_afd_s *pkt = (klvanc_packet_afd_s*)calloc(1, sizeof(*pkt));
	if (!pkt)
		return -ENOMEM;
	...
	pkt->afd = (klvanc_payload_afd_e)afd;
	...
	pkt->barDataFlags = (klvanc_payload_afd_barflags)(sanitizeWord(hdr->payload[3]) >> 4);
	if (pkt->barDataFlags == BARS_TOPBOTTOM) {
...
{
	struct klvanc_packet_afd_s *p = (klvanc_packet_afd_s*)calloc(1, sizeof(*p));
	if (p == NULL)
		return -ENOMEM;
...
{
	if (!klvanc_verify_afd_value((klvanc_payload_afd_e)val))
		return -1;

	pkt->afd = (klvanc_payload_afd_e)val;
	return 0;
}
