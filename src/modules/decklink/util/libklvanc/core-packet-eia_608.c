...
{
	struct klvanc_packet_eia_608_s *pkt = (klvanc_packet_eia_608_s*)p;
...
int parse_EIA_608(struct klvanc_context_s *ctx, struct klvanc_packet_header_s *hdr, void **pp)
{
	...
	struct klvanc_packet_eia_608_s *pkt = (klvanc_packet_eia_608_s*)calloc(1, sizeof(*pkt));
	if (!pkt)
		return -ENOMEM;
...
{
	struct klvanc_packet_eia_608_s *p = (klvanc_packet_eia_608_s*)calloc(1, sizeof(*p));
	if (p == NULL)
		return -ENOMEM;
