...
{
	struct klvanc_packet_eia_708b_s *p = (klvanc_packet_eia_708b_s*)calloc(1, sizeof(*p));
	if (p == NULL)
		return -ENOMEM;
...
{
	struct klvanc_packet_eia_708b_s *pkt = (klvanc_packet_eia_708b_s*)p;
...
int parse_EIA_708B(struct klvanc_context_s *ctx, struct klvanc_packet_header_s *hdr, void **pp)
{
	...
	struct klvanc_packet_eia_708b_s *pkt = (klvanc_packet_eia_708b_s*)calloc(1, sizeof(*pkt));
	if (!pkt) {
		klbs_free(bs);
		return -ENOMEM;
	}
