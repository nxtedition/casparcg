...
{
	struct klvanc_packet_kl_u64le_counter_s *p = (klvanc_packet_kl_u64le_counter_s*)calloc(1, sizeof(*p));
	if (p == NULL)
		return -ENOMEM;
...
int klvanc_dump_KL_U64LE_COUNTER(struct klvanc_context_s *ctx, void *p)
{
	...
	struct klvanc_packet_kl_u64le_counter_s *pkt = (klvanc_packet_kl_u64le_counter_s*)p;
...
int parse_KL_U64LE_COUNTER(struct klvanc_context_s *ctx, struct klvanc_packet_header_s *hdr, void **pp)
{
	...
	struct klvanc_packet_kl_u64le_counter_s *pkt = (klvanc_packet_kl_u64le_counter_s*)calloc(1, sizeof(*pkt));
	if (!pkt)
		return -ENOMEM;
