...
{
	struct klvanc_packet_scte_104_s *pkt = (klvanc_packet_scte_104_s*)p;
		   ...
		   struct klvanc_packet_header_s *hdr, void **pp)
{
	...
	struct klvanc_packet_scte_104_s *pkt = (klvanc_packet_scte_104_s*)calloc(1, sizeof(*pkt));
	if (!pkt)
		return -ENOMEM;
			       ...
			       struct klvanc_multiple_operation_message_operation **op)
{
	struct klvanc_multiple_operation_message *mom = &pkt->mo_msg;
	mom->num_ops++;
	mom->ops = (klvanc_multiple_operation_message_operation*)realloc(mom->ops,
