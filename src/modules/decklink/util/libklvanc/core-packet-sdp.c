...
int klvanc_dump_SDP(struct klvanc_context_s *ctx, void *p)
{
	if (ctx->verbose)
		PRINT_DEBUG("%s()\n", __func__);
	struct klvanc_packet_sdp_s *pkt = (klvanc_packet_sdp_s*)p;
...
//		if (ctx->verbose)
	...
	struct klvanc_packet_sdp_s *pkt = (klvanc_packet_sdp_s*)calloc(1, sizeof(*pkt));
	if (!pkt)
		return -ENOMEM;
	...
	pkt->format_code = (klvanc_sdp_format_code_e)(hdr->payload[3]&0xff);
...
{
	struct klvanc_packet_sdp_s *p = (klvanc_packet_sdp_s*)calloc(1, sizeof(*p));
	if (p == NULL)
		return -ENOMEM;
