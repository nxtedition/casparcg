	...
	struct klvanc_packet_header_s **hdr)
{
	...
	struct klvanc_packet_header_s *p = (klvanc_packet_header_s*)calloc(1, sizeof(struct klvanc_packet_header_s));
	if (!p)
		return -ENOMEM;
...
//			if (ret == KLAPI_OK) {
//				if (ctx->verbose == 2) {
//					ret = dumpByType(ctx, (klvanc_packet_header_s*)decodedPacket);
//					if (ret < 0) {
        ...
        uint32_t bitDepth)
{
	...
	uint16_t *arr = (uint16_t*)calloc(2, srcByteCount + header_length);
...
{
	*dst = (klvanc_packet_header_s*)malloc(sizeof(*src));
	if (*dst == NULL)
		return -ENOMEM;
                       ...
                       int lineNr, int did)
{
	...
	static int idx = 0;
	char *fn = (char*)malloc(strlen(dir) + 128);
