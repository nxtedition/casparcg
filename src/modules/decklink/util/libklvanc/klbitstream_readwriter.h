 ...
 * @param[in]   uint32_t *buf  Buffer size in bytes.
 */
static void klbs_read_set_buffer(struct klbs_context_s *ctx, uint8_t *buf, uint32_t lengthBytes)
{
...
{
	struct klbs_context_s *ctx = (klbs_context_s*)calloc(1, sizeof(struct klbs_context_s));
	if (!ctx)
		return NULL;
