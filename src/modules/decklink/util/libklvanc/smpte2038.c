...
static int smpte2038_parse_pes_payload_int(struct klbs_context_s *bs, struct klvanc_smpte2038_anc_data_packet_s *h)
{
	...
	while (rem > 4) {
		h->lines = (klvanc_smpte2038_anc_data_line_s*)realloc(h->lines, (h->lineCount + 1) * sizeof(struct klvanc_smpte2038_anc_data_line_s));
		 ...
		 */
		l->user_data_words = (uint16_t*)calloc(sizeof(uint16_t), VANC8(l->data_count) + 1);
...
int klvanc_smpte2038_parse_pes_payload(uint8_t *payload, unsigned int byteCount, struct klvanc_smpte2038_anc_data_packet_s **result)
{
	...
	struct klvanc_smpte2038_anc_data_packet_s *h = (klvanc_smpte2038_anc_data_packet_s*)calloc(sizeof(*h), 1);
	if (h == NULL) {
		klbs_free(bs);
		return -1;
	}
...
{
	struct klvanc_smpte2038_packetizer_s *p = (klvanc_smpte2038_packetizer_s*)calloc(1, sizeof(*p));
	if (!p)
		return -1;
...
static void klvanc_smpte2038_buffer_adjust(struct klvanc_smpte2038_packetizer_s *ctx, uint32_t newsizeBytes)
{
	...
	ctx->buf = (uint8_t*)realloc(ctx->buf, newsizeBytes);
...
int klvanc_smpte2038_convert_line_to_words(struct klvanc_smpte2038_anc_data_line_s *l, uint16_t **words, uint16_t *wordCount)
{
	...
	uint16_t *arr = (uint16_t*)malloc((7 + VANC8(l->data_count)) * sizeof(uint16_t));
	if (!arr)
		return -ENOMEM;
